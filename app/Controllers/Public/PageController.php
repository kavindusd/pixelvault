...
HTML;
    ...
    public function removePayment(array $params = []): void
    {
        $user = $this->auth->user();
        if (!$user) {
            redirect('/login');
        ...
        $id = (int) ($_POST['id'] ?? 0);
        if ($id <= 0) {
            redirect('/profile?tab=payments&status=error&msg=' . urlencode('Invalid method ID.'));
        }

        $success = $this->auth->removePaymentMethod($id);
        if ($success) {
            redirect('/profile?tab=payments&status=success&msg=' . urlencode('Payment method removed.'));
        } else {
            redirect('/profile?tab=payments&status=error&msg=' . urlencode('Failed to remove payment method.'));
        }
    }
