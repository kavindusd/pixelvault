...
final class MarketplaceModel
{
    ...
    {
        $stmt = $this->db->prepare('SELECT id, name, email, password_hash, avatar_url, paypal_email, payhere_id, verification_code, token_expires_at FROM users WHERE email = :email LIMIT 1');
    ...
    {
        $stmt = $this->db->prepare('SELECT id, name, email, password_hash, avatar_url, paypal_email, payhere_id, verification_code, token_expires_at FROM users WHERE id = :id LIMIT 1');
        $stmt->execute(['id' => $id]);
        $row = $stmt->fetch();
    ...
    public function updateUserPaymentMethods(int $userId, ?string $paypalEmail, ?string $payhereId): bool
    {
        $stmt = $this->db->prepare(
            'UPDATE users SET paypal_email = :paypal, payhere_id = :payhere WHERE id = :id'
        );
        return $stmt->execute([
            'paypal' => $paypalEmail,
            'payhere' => $payhereId,
