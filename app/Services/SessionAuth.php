...
final class SessionAuth
{
    ...
    public function linkPaymentMethod(string $type, string $value): bool
    {
        ...
        $success = $this->marketplace->addUserPaymentMethod($userId, $type, $value, true);
        if ($success) {
    ...
    public function removePaymentMethod(int $methodId): bool
    {
        $user = $this->user();
        if (!$user) return false;

        $userId = (int) $user['id'];
        $success = $this->marketplace->deleteUserPaymentMethod($userId, $methodId);
        if ($success) {
            $this->refreshSessionUser($userId);
            return true;
        }
    ...
    {
        $updatedUser = $this->marketplace->findUserById($userId);
        if ($updatedUser) {
            $_SESSION['user'] = $this->toSessionUser($updatedUser);
        }
    }
    ...
    private function toSessionUser(array $user): array
    {
        ...
        return [
            ...
            'paymentMethods' => $this->marketplace->getUserPaymentMethods($userId),
