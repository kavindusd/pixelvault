      ...
      <div class="space-y-8">
        ...
        <div class="flex flex-col sm:flex-row gap-4">
          ...
          <?php elseif ($limitReached): ?>
            <form method="POST" action="/request-extension" class="flex-1">
              <input type="hidden" name="product_id" value="<?= e((string) ($product['id'] ?? '')) ?>">
              <input type="hidden" name="message" value="Hi, I have reached my download limit for this product (<?= e((string)($product['name'] ?? '')) ?>) and would like to request an extension for future updates. Thank you!">
              <button type="submit" class="w-full bg-blue-500 text-white py-5 rounded-2xl font-bold text-lg shadow-glow flex items-center justify-center gap-3 hover:brightness-110 active:scale-[0.98] transition-all">
                <?= icon_svg('ArrowUpRight', 'h-5 w-5') ?> Request Extension
              </button>
            </form>
          <?php endif; ?>
