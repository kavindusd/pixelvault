  ...
  <div class="flex-1 overflow-y-auto relative z-10 custom-scrollbar h-[calc(100vh-80px)] lg:h-screen lg:pl-32">
    <div class="p-6 md:p-12 lg:p-20 max-w-7xl mx-auto relative pb-32">
                  ...
                  ?>
                    <div class="grid grid-cols-12 gap-4 px-8 py-7 items-center hover:bg-secondary/20 transition-colors group">
                      ...
                      <div class="col-span-2 text-right">
                        <?php if ($remaining > 0): ?>
                          <a href="/product/<?= e((string)($item['id'] ?? '')) ?>" class="inline-flex items-center gap-2 h-11 px-5 rounded-2xl bg-ink text-ink-foreground hover:shadow-glow text-xs font-bold transition-all">View Product</a>
                        <?php else: ?>
                          <form method="POST" action="/request-extension" class="inline-block">
                            ...
                            <button type="submit" class="inline-flex items-center gap-2 h-11 px-5 rounded-2xl bg-blue-500 text-white hover:shadow-glow text-xs font-bold transition-all active:scale-[0.98]">
                              <?= icon_svg('ArrowUpRight', 'h-3 w-3') ?> Request Extension
                            </button>
                          </form>
                        <?php endif; ?>
                      </div>
        ...
        <?php elseif ($activeTab === 'payments'): 
          $hasPaypal = !empty($user['paypal_email']);
          $hasPayhere = !empty($user['payhere_id']);
        ?>
              ...
              <div class="group relative rounded-[2.5rem] border border-border bg-card overflow-hidden shadow-soft hover:shadow-glow transition-all duration-500">
                ...
                <div class="p-8 relative z-10">
                  ...
                  <div class="space-y-1 mb-8">
                    ...
                    <?php if ($hasPaypal): 
                      $email = (string) $user['paypal_email'];
                      $atPos = strpos($email, '@');
                      $masked = substr($email, 0, 1) . str_repeat('*', $atPos - 2) . substr($email, $atPos - 1);
                    ?>
                  ...
                  <div class="flex items-center justify-between">
                    ...
                    <?php if ($hasPaypal): ?>
                      <form action="/profile/link-payment" method="POST" onsubmit="return confirm('Disconnect PayPal?')">
              ...
              <div class="group relative rounded-[2.5rem] border border-border bg-card overflow-hidden shadow-soft hover:shadow-glow transition-all duration-500">
                ...
                <div class="p-8 relative z-10">
                  ...
                  <div class="space-y-1 mb-8">
                    ...
                    <?php if ($hasPayhere): 
                      $id = (string) $user['payhere_id'];
                      $maskedId = substr($id, 0, 4) . str_repeat('*', 6) . substr($id, -2);
                    ?>
                  ...
                  <div class="flex items-center justify-between">
                    ...
                    <?php if ($hasPayhere): ?>
                      <form action="/profile/link-payment" method="POST" onsubmit="return confirm('Revoke Payhere access?')">
                        <input type="hidden" name="type" value="payhere">
                        <input type="hidden" name="value" value="">
...
<div id="paypalModal" class="fixed inset-0 z-[200] hidden flex items-center justify-center p-6">
  ...
  <div class="relative w-full max-w-md bg-white text-zinc-900 rounded-[2.5rem] overflow-hidden shadow-2xl animate-scale-in">
    <div class="p-8 text-center border-b border-zinc-100">
      ...
      <div class="mt-6 p-4 rounded-2xl bg-zinc-50 text-left border border-zinc-200">
         <p class="text-[10px] font-bold uppercase tracking-widest text-zinc-400 mb-2">Test Credentials</p>
         <p class="text-xs font-medium">Email: <span class="select-all font-bold">sb-12345@business.example.com</span></p>
         <p class="text-xs font-medium mt-1">Pass: <span class="select-all font-bold">password123</span></p>
      </div>
    </div>
...
<script>
  ...
  function handlePaypalLogin(e) {
      ...
      const pass = document.getElementById('paypalPass').value;
      const btn = e.target.querySelector('button');
      const loading = document.getElementById('paypalLoading');

      if (email === 'sb-12345@business.example.com' && pass === 'password123') {
          ...
          alert('Invalid test credentials. Please use the ones provided above.');
      }
  }
