...
/**
 * Render the site logo as an <img> when a logo is uploaded,
 * or fall back to the branded letter-box.
 *
 * @param string $imgClass   CSS classes applied to the <img> element.
 * @param string $boxClass   CSS classes applied to the fallback <div> box.
 */
function site_logo_html(
    string $imgClass = 'h-8 w-auto object-contain',
    string $boxClass = 'h-8 w-8 rounded-lg bg-primary text-primary-foreground flex items-center justify-center shadow-glow shrink-0'
): string {
    $logo     = (string) site_config('site_logo', '');
    $siteName = (string) site_config('site_name', 'PixelVault');
    $altText  = e($siteName);

    if ($logo !== '') {
        return '<img src="' . e($logo) . '" alt="' . $altText . '" class="' . e($imgClass) . '">';
    }

    // Fallback: first letter of the site name inside a coloured box
    $initial = strtoupper(substr(strip_tags($siteName), 0, 1)) ?: 'P';
    return '<div class="' . e($boxClass) . '"><span class="font-display font-black text-xl leading-none">' . e($initial) . '</span></div>';
}